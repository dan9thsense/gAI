...
class Agent():
    def __init__(self, learningRate, numberOfStates, numberOfActions):
        ...
        hidden = slim.fully_connected(state_in_OH, numberOfActions,\
            biases_initializer=None,activation_fn=tf.nn.sigmoid, \
               weights_initializer=tf.ones_initializer(), scope='hidden_layer')

        self.hiddenAction_holder = tf.placeholder(shape=[1],dtype=tf.int32, name='hiddenAction')
        self.hiddenOutput = tf.reshape(hidden,[-1])
        ...
        hiddenState_in_OH = slim.one_hot_encoding(hiddenAction, numberOfStates)

        '''
        outputVector = slim.fully_connected(hiddenState_in_OH, numberOfActions,\
            biases_initializer=None,activation_fn=tf.nn.sigmoid, \
        ...
        with tf.name_scope('calculations'):
            self.loss = -(tf.log(self.selected_hiddenOutput)*self.reward_holder)
    ...
    def runNet(self):
        ...
        while True:
            ...
            if self.reward < 1 and np.random.rand(1) < self.e:
                ...
                self.hiddenAction = self.sess.run(self.myAgent.chosen_hiddenAction,\
                    feed_dict={self.myAgent.state_in:[self.currentState]})
            yield self.characters[self.hiddenAction]
            # we freeze here
            ...
            #Update the network
            networkFeeder = {self.myAgent.reward_holder:[self.reward],self.myAgent.hiddenAction_holder:[self.hiddenAction],\
            ...
            _, self.weightValues, loss, output, selOutput, self.statesToActions = self.sess.run([self.myAgent.update, self.weights,\
                self.myAgent.loss, self.myAgent.hiddenOutput, self.myAgent.selected_hiddenOutput, self.states_to_actions],\
                    feed_dict=networkFeeder)

            if self.useTensorBoard:
                summaryFeeder = {self.myAgent.reward_holder:[self.reward],self.myAgent.action_holder:[self.action],\
                self.weights:self.weightValues, self.myAgent.loss:loss, self.myAgent.hiddenOutput:output,\
                self.myAgent.selected_hiddenOutput:selOutput }
