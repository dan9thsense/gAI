...
class Agent():
    def __init__(self, learningRate, numberOfStates, numberOfActions):
        ...
        with tf.name_scope('input'):
            ...
            self.state_in_OH = slim.one_hot_encoding(self.state_in, numberOfStates)
            print('stateinOH= ', self.state_in_OH)
            #valueIn = tf.argmax(state_in_OH)
            self.reward_holder = tf.placeholder(shape=[1],dtype=tf.float32, name='reward')

        #the form of hidden is [ [ result_1, result_2, ... result_numberOfActions ] ]
        #hidden.shape = (1,numberOfActions)
        #tf.shape(hidden) = (2,) because you use 2 indices to get a value, the first of which is always [0]
        #the only row, hidden[0], has shape = (1,) which means it has a single row with 1 entry, index [0]
        #hidden[0]'s only row, has length numberOfActions, it contains the results for each possible action
        #that is, it has a value for (activation_function(Wx + b)) in each entry
        #self.hidden = slim.fully_connected(self.state_in_OH, 71,\
        #    biases_initializer=tf.constant_initializer(0.1),activation_fn=tf.nn.elu, \
        #        weights_initializer=tf.ones_initializer(), scope='hidden')
        #self.hidden = tf.nn.dropout(self.hidden1,0.8)
        #self.hidden_output = tf.reshape(hidden,[-1])
        #for each row of weights (each state), find the index of the largest value
        #that index corresponds to the action we would take for that state
        #self.hidden_action_array = tf.argmax(self.hidden, 0) #array of all 0's since hidden only has 1 column
        #self.hidden_action = tf.argmax(self.hidden, 1) #index of the largest value in hidden
        #self.hidden_action = self.hidden_action_array[0]

        #tf.reshape(self.hidden_action, [-1, 71])
        #self.hidden_state_in_OH = slim.one_hot_encoding(self.hidden_action, numberOfActions)
        #print('hiddenstateinOH = ', self.hidden_state_in_OH)

        #using the elu or relu activation function works, but sigmoid prevents any learning.
        outputVector = slim.fully_connected(self.state_in_OH, numberOfActions,\
            biases_initializer=None,activation_fn=None, \
                weights_initializer=tf.ones_initializer(), scope='layer1')
        #if you want to keep the values near 0, can take oout most of the mean
        # if you don't leave some residual, the it doesn't work, as all the initial weights become 0
        #outputVector = outputV + 0.1 - tf.reduce_mean(outputV)
        ...
        self.Wx = tf.matmul(self.tvars[0],tf.transpose(self.state_in_OH))
        #self.hidden_weight_action_array = tf.argmin(self.tvars[0], 0) #array of the largest weight row
        #self.hidden_weight_action = tf.argmin(self.tvars[0], 1) #index of the largest weight column
...
class simpleAgent(Responder):
    ...
    def createTensorboardSummaries(self):
        ...
        tf.summary.scalar('hidden_scalar0', self.myAgent.hidden[0][0])
        tf.summary.scalar('hidden_scalar41', self.myAgent.hidden[0][41])
        tf.summary.scalar('hidden_scalar50', self.myAgent.hidden[0][50])
        tf.summary.histogram('hidden', self.myAgent.hidden)
    ...
    def runNet(self):
        ...
        while True:
            ...
            if self.reward < 1 and np.random.rand(1) < self.e:
                ...
                #hidden, hidden_action_array, hidden_action, self.action = self.sess.run([ \
                #    self.myAgent.hidden,\
                #    self.myAgent.hidden_action_array, self.myAgent.hidden_action, self.myAgent.chosen_action],\
            ...
            #print('loss = ', loss)
            #print('tf.shape tvars = ', tf.shape(tvars[1]))
            #tvarsArray = np.array(tvars)
            #print('tvars.shape = ', tvars.shape)

            #print('len hidden = ', len(hidden), 'len hidden0 = ', len(hidden[0]))
            #print('tf.shape hidden = ', tf.shape(hidden), ' tf.shape hidden0 = ', tf.shape(hidden[0]))
            #print('shape hidden = ', hidden.shape, ' shape hidden0 = ', hidden[0].shape)
            #print('current state = ', self.currentState)
            #print('self.action = ', self.action)
            #print('output action hidden = ', hidden[0][self.currentState])
            #print('ouput values for all actions: ', hidden[0])
            #print('hidden action = ', hidden_action[0])
            #print('output action hidden array = ', hidden_action_array)
            #print('ouput values for all actions: ', hidden[0])
            #print('hidden action = ', hidden_action)

            #print('hidden_action = ', hidden_action, 'self.action = ', self.action)
            ...
            inputOH, tvars, rewardVal, self.weightValues, loss, output, selOutput, self.statesToActions = self.sess.run([\
                self.myAgent.state_in_OH, self.myAgent.tvars, self.myAgent.reward_holder, self.weights,\
            ...
            #print('hidden = ', hidden)
            print('output = ', output)
            #print('tvars: ', self.myAgent.tvars)

            #print('tvars hidden weights: ', tvars[0])

            #Wx, hidden_weight_action_array, hidden_weight_action = self.sess.run([\
            #    self.myAgent.hidden_weight_action_array, self.myAgent.hidden_weight_action],\
            #    feed_dict={self.myAgent.tvars[0]:tvars[0], self.myAgent.state_in_OH:inputOH})
            Wx = np.matmul(tvars[0], np.transpose(inputOH))
            print('Wx = ', np.transpose(Wx))
            #print('weight action hidden array = ', hidden_weight_action_array)
            #print('ouput values for all actions: ', hidden[0])
            #print('hidden weight action = ', hidden_weight_action)
            print('selOutput = ', selOutput)
            print('reward = ', rewardVal)
            print('loss = ', loss)
            #print('update = ', update)
            _ = self.sess.run(self.myAgent.update, feed_dict=networkFeeder)
            if self.useTensorBoard:
                ...
                self.myAgent.selected_output:selOutput, self.myAgent.hidden:hidden }
