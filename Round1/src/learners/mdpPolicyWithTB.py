...
class mdpPolicyAgent(Responder):
    ...
    def createGraph(self):
        ...
        self.lossTB = tf.summary.scalar("loss", self.myAgent.loss)
        # use tensorBoard, writing to the directory "output"
        self.writer = tf.summary.FileWriter("output", self.sess.graph)
        self.learningRateTB = tf.summary.scalar("learning_rate", self.learningRate)
    ...
    def runNet(self):
        ...
        while True:
           # first check if the call came from reset or done
            ...
            a_dist = self.sess.run(self.myAgent.output,feed_dict={self.myAgent.state_in:stateRS})
            ...
            if self.reward == 1:
                ...
                grads = self.sess.run(self.myAgent.gradients, feed_dict=feed_dict)
                #grads = self.sess.run(self.myAgent.gradients, feed_dict=feed_dict)
                #self.writer.add_summary(lossSummary, counter)
