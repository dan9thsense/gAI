...
class Agent():
    def __init__(self, learningRate, numberOfStates, numberOfActions):
        ...
        self.hidden = slim.fully_connected(state_in_OH, numberOfActions,\
            biases_initializer=tf.constant_initializer(0.1),activation_fn=tf.nn.relu, \
               weights_initializer=tf.ones_initializer())

        #self.hiddenAction_holder = tf.placeholder(shape=[1],dtype=tf.int32, name='hiddenAction')
        #flattens hidden to be 1 dimensional
        self.hiddenOutput = tf.reshape(self.hidden,[-1])
        #self.selected_hiddenOutput = tf.slice(self.hiddenOutput,self.hiddenAction_holder,[1])
        self.chosen_hiddenAction = tf.argmax(self.hiddenOutput,0)
        hiddenAction = tf.reshape(self.chosen_hiddenAction,[-1])
        self.hiddenState_in_OH = slim.one_hot_encoding(hiddenAction, numberOfStates)

        #outputVector = slim.fully_connected(hidden, numberOfActions,\
        #    biases_initializer=None,activation_fn=tf.nn.sigmoid, \
        #        weights_initializer=tf.ones_initializer(), scope='layer1')

        outputVector = slim.fully_connected(self.hidden, numberOfActions,\
            biases_initializer=tf.constant_initializer(0.1),activation_fn=tf.nn.sigmoid, \
                weights_initializer=tf.ones_initializer())
    ...
    def runNet(self):
        ...
        while True:
            ...
            if self.reward < 1 and np.random.rand(1) < self.e:
                ...
                #self.action, hiddenAction = self.sess.run([self.myAgent.chosen_action, self.myAgent.chosen_hiddenAction],\
                #    feed_dict={self.myAgent.state_in:[self.currentState]})
                self.action = self.sess.run(self.myAgent.chosen_action,\
                    feed_dict={self.myAgent.state_in:[self.currentState]})
            #print('hiddenAction = ', hiddenAction)
            ...
            _, self.weightValues, loss, output, hiddenOutput, hiddenStateOH, selOutput, self.statesToActions = self.sess.run([self.myAgent.update, self.weights,\
                self.myAgent.loss, self.myAgent.output, self.myAgent.hiddenOutput, self.myAgent.hiddenState_in_OH, self.myAgent.selected_output, self.states_to_actions],\
                    feed_dict=networkFeeder)

            #print(self.statesToActions)
            print(hiddenStateOH)
            print("output =", output)
            if self.useTensorBoard:
